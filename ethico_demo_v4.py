...
def sync_to_google_sheets(zone, scope, ent_id, hens, cocks, water, feed, task, notes):
    try:
        ...
        creds = ServiceAccountCredentials.from_json_keyfile_name(
            "creds.json", scope_api
        )
        client = gspread.authorize(creds)
        sheet = client.open("EthiCoAI_Master_Log").sheet1

        # Mapping to your 10 columns:
        # [Timestamp, Zone, Scope, Entity_ID, Hens, Cocks, Water, Feed, Task, Notes]
        row = [
...
with main_col:
    ...
    if mode == "🚜 Real-Time Farm Twin":
        ...
        if st.session_state.active_animal:
            ...
            with c1:
                # ID becomes Zone_Name as requested
            ...
            with c2:
                ...
                if st.button("🚀 Sync to Master Ledger"):
                    if notes_input:
                        # Passing values to match your 10-column schema:
                        # zone, scope, ent_id, hens, cocks, water, feed, task, notes
                        success = sync_to_google_sheets(
                            zone=selected_zone,
                            scope=species_choice,
                            ent_id=full_id,
                            hens="N/A",  # Placeholder as this is a single animal profile
                            cocks="N/A",  # Placeholder
                            water="Optimum",
                            feed=high_prot_feed,
                            task=task_choice,
                            notes=notes_input,
        ...
        if st.button("🔄 Refresh Cloud Data"):
            try:
                ...
                if data:
                    ...
                    # Show only the last 5 entries, reversed so newest is on top
        ...
        if st.button("Deposit to Vault & Sync to Cloud"):
            if coll > 0:
                ...
                # 2. Sync to 10-column Google Sheet
                # Schema: [Timestamp, Zone, Scope, Entity_ID, Hens, Cocks, Water, Feed, Task, Notes]
                success = sync_to_google_sheets(
                    zone="Inventory_Vault",
                    scope="Production",
                    ent_id=f"VAULT_{who}",
                    hens=coll,  # Putting Egg count under Hens for tallying
                    cocks=0,
                    water="N/A",
                    feed="N/A",
                    task="Egg Collection",
        ...
        with col_y:
            st.markdown(
                """
                <div style='writing-mode: vertical-rl; text-orientation: mixed; height: 350px; font-weight: bold; font-size: 18px; color: #1e293b; padding-top: 50px; border-right: 2px solid #cbd5e1;'>
        ...
        with col_chart:
            ...
            st.markdown(
                """
                <div style='text-align: center; font-weight: bold; font-size: 18px; color: #1e293b; margin-top: -10px; border-top: 2px solid #cbd5e1; padding-top: 5px;'>
                    24-HOUR CIRCADIAN CYCLE (TIME)
                </div>
            """,
                unsafe_allow_html=True,
            )
...
# streamlit run ethico_demo_v4.py
